...
import './mount'
import { filterWithCache, filterWithSearchResult, updateHits, Project, ResultItem, COMMAND, getOpenCommand, setProjectApp } from 'cheetah-core'
import { initCore, output, chooseFile, commandMap, getAllDefaultApp, errorHandle } from './common'
 ...
 * @return {*}
 */
async function search(action: any, keyword: string, callbackSetList: any): Promise<void> {
  try {
    initCore()
...
window.exports = {
  open: {
    ...
    args: {
      search,
      enter: (action: any, callbackSetList: any) => search(action, '', callbackSetList),
      select: async (action: any, itemData: ResultItem) => {
        try {
          ...
          const command = await getOpenCommand(itemData, commandType, defaultAppPath)
